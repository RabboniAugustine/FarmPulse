...
interface Flock {
  ...
  ageWeeks: number; hatchDate: string; dateAcquired: string; notes: string;
}
...
interface ActivityEntry {
  id: string; date: string;
  type: "death" | "status_change" | "sale" | "vaccination" | "expense" | "added" | "updated" | "deleted" | "note";
  subject: string;
  description: string;
  details: string;
  performedBy: string;
...
const activityIcon: Record<ActivityEntry["type"], React.ReactNode> = {
  ...
  updated: <RefreshCw size={14} className="text-blue-600" />,
...
const activityBg: Record<ActivityEntry["type"], string> = {
  ...
  deleted: "border-border bg-muted/40",
  note: "border-border bg-card",
};

function ActivitySection({ activity, onAdd }: {
  activity: ActivityEntry[]; onAdd: (a: Omit<ActivityEntry, "id" | "performedBy">) => void;
}) {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    onAdd({ date: form.date, type: form.type as ActivityEntry["type"], subject: form.subject, description: form.description, details: form.details });
    setShowModal(false);
          ...
          <div key={a.id} className={`flex gap-3 p-3 border ${activityBg[a.type]}`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                ...
                {a.performedBy && <span className="text-xs font-mono text-muted-foreground/70 ml-auto">by {a.performedBy}</span>}
              </div>
            </div>
          </div>
        ))}
...
// ─── Poultry Section ──────────────────────────────────────────────────────────
function PoultrySection({ flocks, onAdd, onEdit, onDelete, onLogDeath }: {
  flocks: Flock[]; onAdd: (f: Flock) => void; onEdit: (id: string, changes: Partial<Flock>) => void; onDelete: (id: string) => void;
  ...
  const [form, setForm] = useState({ name: "", breed: "", count: "", purpose: "layers", hatchDate: "", dateAcquired: "", notes: "" });
  const [editForm, setEditForm] = useState({ name: "", breed: "", count: "", purpose: "layers", hatchDate: "", dateAcquired: "", notes: "" });
  ...
  const displayWeeks = (f: Flock) => f.hatchDate ? weeksFromDate(f.hatchDate) ?? 0 : f.ageWeeks;

  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    onAdd({ id: uid(), name: form.name, breed: form.breed, count: parseInt(form.count)||0, purpose: form.purpose as Flock["purpose"], ageWeeks: 0, hatchDate: form.hatchDate, dateAcquired: form.dateAcquired, notes: form.notes });
    setShowModal(false);
    setForm({ name: "", breed: "", count: "", purpose: "layers", hatchDate: "", dateAcquired: "", notes: "" });
  };

  const openEdit = (f: Flock) => {
    setEditModal(f);
    setEditForm({ name: f.name, breed: f.breed, count: f.count.toString(), purpose: f.purpose, hatchDate: f.hatchDate, dateAcquired: f.dateAcquired, notes: f.notes });
  };

  const submitEdit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!editModal) return;
    onEdit(editModal.id, { name: editForm.name, breed: editForm.breed, count: parseInt(editForm.count)||0, purpose: editForm.purpose as Flock["purpose"], hatchDate: editForm.hatchDate, dateAcquired: editForm.dateAcquired, notes: editForm.notes });
    setEditModal(null);
  };
  ...
  return (
    <div className="p-6">
          ...
          <tbody>
            {flocks.map(f => {
              ...
              <tr key={f.id} className="hover:bg-muted/30 transition-colors">
                ...
                <TD mono>
                  {wk < 52 ? `${wk}wk` : `${Math.floor(wk/52)}yr ${wk%52}wk`}
                  {!f.hatchDate && <span title="Set a hatch date to auto-update this weekly" className="ml-1 text-muted-foreground">*</span>}
                </TD>
                <TD mono>{fmtDate(f.dateAcquired)}</TD>
                <TD><span className="text-xs text-muted-foreground">{f.notes || "—"}</span></TD>
                <TD>
                  <div className="flex items-center gap-1">
                    <button onClick={() => openEdit(f)} title="Edit flock" className="text-muted-foreground hover:text-foreground transition-colors">
                      <RefreshCw size={13} />
                    </button>
                    <button
      ...
      {flocks.some(f => !f.hatchDate) && (
        <p className="text-xs text-muted-foreground mt-2 font-mono">* No hatch date set — edit the flock to add one so age updates automatically every week.</p>
      )}
      ...
      {showModal && (
        <Modal title="Add Flock" onClose={() => setShowModal(false)}>
          <form onSubmit={submit} className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <FormField label="Hatch Date"><input type="date" className={inputCls} value={form.hatchDate} onChange={e => set("hatchDate", e.target.value)} /></FormField>
            ...
            <p className="text-xs text-muted-foreground font-mono -mt-1">Age is calculated automatically from the hatch date and updates every week.</p>
      ...
      {editModal && (
        <Modal title={`Edit Flock — ${editModal.name}`} onClose={() => setEditModal(null)}>
          <form onSubmit={submitEdit} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <FormField label="Flock Name"><input required className={inputCls} value={editForm.name} onChange={e => setE("name", e.target.value)} /></FormField>
              <FormField label="Breed"><input required className={inputCls} value={editForm.breed} onChange={e => setE("breed", e.target.value)} /></FormField>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <FormField label="Count"><input required type="number" min="0" className={inputCls} value={editForm.count} onChange={e => setE("count", e.target.value)} /></FormField>
              <FormField label="Purpose">
                <select className={selectCls} value={editForm.purpose} onChange={e => setE("purpose", e.target.value)}>
                  <option value="layers">Layers</option>
                  <option value="broilers">Broilers</option>
                  <option value="dual-purpose">Dual-Purpose</option>
                </select>
            ...
            <div className="grid grid-cols-2 gap-3">
              <FormField label="Hatch Date"><input type="date" className={inputCls} value={editForm.hatchDate} onChange={e => setE("hatchDate", e.target.value)} /></FormField>
              <FormField label="Date Acquired"><input type="date" className={inputCls} value={editForm.dateAcquired} onChange={e => setE("dateAcquired", e.target.value)} /></FormField>
            </div>
            <p className="text-xs text-muted-foreground font-mono -mt-1">Setting a hatch date switches this flock to automatic weekly age tracking.</p>
            <FormField label="Notes"><textarea className={textareaCls} rows={2} value={editForm.notes} onChange={e => setE("notes", e.target.value)} /></FormField>
            <div className="flex justify-end gap-2 pt-2">
...
// ─── Rabbits Section ──────────────────────────────────────────────────────────
function RabbitsSection({ rabbits, cages, onAdd, onEdit, onDelete, onUpdateStatus, onAddCage, onDeleteCage, onAssignCage }: {
  rabbits: RabbitRecord[]; cages: Cage[]; onAdd: (r: RabbitRecord) => void;
  onEdit: (id: string, changes: Partial<RabbitRecord>) => void; onDelete: (id: string) => void;
  onUpdateStatus: (id: string, status: RabbitRecord["status"], notes: string, lastBredDate: string, expectedKindleDate: string) => void;
  ...
  const [editForm, setEditForm] = useState({ breed: "", sex: "female", dob: "", weightKg: "" });
  ...
  const openEdit = (r: RabbitRecord) => {
    setEditModal(r);
    setEditForm({ breed: r.breed, sex: r.sex, dob: r.dob, weightKg: r.weightKg.toString() });
  };
  const submitEdit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!editModal) return;
    onEdit(editModal.id, { breed: editForm.breed, sex: editForm.sex as RabbitRecord["sex"], dob: editForm.dob, weightKg: parseFloat(editForm.weightKg) || 0 });
    setEditModal(null);
  };
  ...
  // Rabbit gestation is ~31 days — auto-fill the expected kindle date from
  // the date the doe was crossed, while still letting the user override it.
  const RABBIT_GESTATION_DAYS = 31;
  const setBredDate = (value: string) => {
    setStatusForm(p => ({
      ...p,
      lastBredDate: value,
      expectedKindleDate: value ? addDays(value, RABBIT_GESTATION_DAYS) : p.expectedKindleDate,
    }));
  };
  ...
  const submitStatus = (e: React.FormEvent) => {
    ...
    onUpdateStatus(statusModal.id, statusForm.status as RabbitRecord["status"], statusForm.notes, statusForm.lastBredDate, statusForm.expectedKindleDate);
    setStatusModal(null);
  };
    ...
    <div className="p-6">
      ...
      <div className="bg-card border border-border overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr>
              <TH>Tag ID</TH><TH>Breed</TH><TH>Sex</TH>
              <TH>Status</TH><TH>Cage</TH><TH>Age</TH><TH>Weight</TH>
              <TH>Last Bred</TH><TH>Kindle Date</TH><TH>Actions</TH>
            </tr>
          </thead>
          <tbody>
            {rabbits.map(r => {
              ...
              <tr key={r.id} className={`hover:bg-muted/30 transition-colors ${r.status === "deceased" ? "opacity-50" : ""}`}>
                ...
                <TD>
                  <div className="flex items-center gap-1">
                    <button onClick={() => openEdit(r)} title="Edit breed/sex/DOB/weight" className="text-muted-foreground hover:text-foreground transition-colors">
                      <RefreshCw size={13} />
                    </button>
                    <button
                      onClick={() => openStatus(r)}
      ...
      {editModal && (
        <Modal title={`Edit Rabbit — ${editModal.tagId}`} onClose={() => setEditModal(null)}>
          <p className="text-xs font-mono text-muted-foreground mb-3">Tag ID stays fixed as the rabbit's permanent identifier.</p>
          <form onSubmit={submitEdit} className="space-y-3">
            <FormField label="Tag ID">
              <input disabled className={`${inputCls} opacity-60 cursor-not-allowed`} value={editModal.tagId} />
            </FormField>
            <div className="grid grid-cols-2 gap-3">
              <FormField label="Breed"><input required className={inputCls} value={editForm.breed} onChange={e => setE("breed", e.target.value)} /></FormField>
              <FormField label="Sex">
                <select className={selectCls} value={editForm.sex} onChange={e => setE("sex", e.target.value)}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <FormField label="Date of Birth"><input type="date" className={inputCls} value={editForm.dob} onChange={e => setE("dob", e.target.value)} /></FormField>
              <FormField label="Weight (kg)"><input type="number" step="0.1" min="0" className={inputCls} value={editForm.weightKg} onChange={e => setE("weightKg", e.target.value)} /></FormField>
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <Btn variant="outline" onClick={() => setEditModal(null)}>Cancel</Btn>
              <Btn type="submit">Save Changes</Btn>
      ...
      {statusModal && (
        <Modal title={`Update Status — ${statusModal.tagId}`} onClose={() => setStatusModal(null)}>
          ...
          <form onSubmit={submitStatus} className="space-y-3">
            ...
            {(statusForm.status === "breeding" || statusForm.status === "pregnant") && (
              <div className="grid grid-cols-2 gap-3">
                <FormField label={statusForm.status === "pregnant" ? "Date Pregnant / Crossed" : "Date Bred"}>
                  <input type="date" className={inputCls} value={statusForm.lastBredDate} onChange={e => setBredDate(e.target.value)} />
                </FormField>
            ...
            {statusForm.status === "pregnant" && (
              <p className="text-xs text-muted-foreground font-mono -mt-1">Expected kindle is auto-filled {RABBIT_GESTATION_DAYS} days from the cross date — adjust it if you know the actual date.</p>
            )}
...
export default function App() {
  ...
  const actor = (sessionData?.user?.name as string) || (sessionData?.user?.email as string) || "Unknown user";
  ...
  const addActivity = useCallback((a: Omit<ActivityEntry, "id" | "performedBy">) => {
    setActivity(p => [{ ...a, id: uid(), performedBy: actor }, ...p]);
  }, [actor]);

  const addFlock = useCallback((f: Flock) => {
    setFlocks(p => [f, ...p]);
    addActivity({ date: f.dateAcquired || today(), type: "added", subject: f.name, description: `${f.count} ${f.breed} (${f.purpose}) acquired`, details: `Flock: ${f.name}` });
  }, [addActivity]);

  const editFlock = useCallback((id: string, changes: Partial<Flock>) => {
    const before = flocks.find(f => f.id === id);
    setFlocks(p => p.map(f => f.id === id ? { ...f, ...changes } : f));
    addActivity({ date: today(), type: "updated", subject: before?.name ?? id, description: `Flock ${before?.name ?? id} updated`, details: "" });
  }, [flocks, addActivity]);

  const addRabbit = useCallback((r: RabbitRecord) => {
    setRabbits(p => [r, ...p]);
    addActivity({ date: today(), type: "added", subject: r.tagId, description: `Rabbit ${r.tagId} added (${r.breed})`, details: `${r.sex} · ${r.weightKg}kg` });
  }, [addActivity]);

  const editRabbit = useCallback((id: string, changes: Partial<RabbitRecord>) => {
    const before = rabbits.find(r => r.id === id);
    setRabbits(p => p.map(r => r.id === id ? { ...r, ...changes } : r));
    addActivity({ date: today(), type: "updated", subject: before?.tagId ?? id, description: `Rabbit ${before?.tagId ?? id} details updated`, details: "" });
  }, [rabbits, addActivity]);

  const addPig = useCallback((p: PigRecord) => {
    setPigs(prev => [p, ...prev]);
    addActivity({ date: today(), type: "added", subject: p.tagId, description: `Pig ${p.name || p.tagId} added (${p.breed})`, details: `${p.sex} · ${p.weightKg}kg` });
  }, [addActivity]);

  const addEggLog = useCallback((e: EggLog) => {
    setEggLogs(p => [e, ...p]);
    addActivity({ date: e.date || today(), type: "note", subject: `Egg log`, description: `${e.count} eggs logged (${e.damaged} damaged)`, details: "" });
  }, [addActivity]);
  const addExpense = useCallback((e: Expense) => {
    setExpenses(p => [e, ...p]);
    addActivity({ date: e.date || today(), type: "expense", subject: e.description, description: `Expense logged: ${e.description} — ${fmt(e.amount)}`, details: e.category });
  }, [addActivity]);
  const addSale = useCallback((s: Sale) => {
    setSales(p => [s, ...p]);
    addActivity({ date: s.date || today(), type: "sale", subject: s.product, description: `Sale: ${s.quantity} ${s.unit} ${s.product} — ${fmt(s.quantity * s.pricePerUnit)}`, details: s.buyer ? `Buyer: ${s.buyer}` : "" });
  }, [addActivity]);
  const addVaccination = useCallback((v: Vaccination) => {
    setVaccinations(p => [v, ...p]);
    addActivity({ date: v.date || today(), type: "vaccination", subject: v.animalGroup, description: `${v.vaccine} administered to ${v.animalGroup}`, details: v.notes });
  }, [addActivity]);

  const delFlock = useCallback((id: string) => {
    const f = flocks.find(x => x.id === id);
    setFlocks(p => p.filter(x => x.id !== id));
    addActivity({ date: today(), type: "deleted", subject: f?.name ?? id, description: `Flock ${f?.name ?? id} removed`, details: "" });
  }, [flocks, addActivity]);
  const delRabbit = useCallback((id: string) => {
    const r = rabbits.find(x => x.id === id);
    setRabbits(p => p.filter(x => x.id !== id));
    addActivity({ date: today(), type: "deleted", subject: r?.tagId ?? id, description: `Rabbit ${r?.tagId ?? id} removed`, details: "" });
  }, [rabbits, addActivity]);

  const addCage = useCallback((c: Cage) => {
    setCages(p => [c, ...p]);
    addActivity({ date: today(), type: "added", subject: c.name, description: `Cage ${c.name} added (capacity ${c.capacity})`, details: c.location });
  }, [addActivity]);
  const delCage = useCallback((id: string) => {
    const c = cages.find(x => x.id === id);
    setCages(p => p.filter(x => x.id !== id));
    setRabbits(p => p.map(r => r.cageId === id ? { ...r, cageId: null } : r));
    addActivity({ date: today(), type: "deleted", subject: c?.name ?? id, description: `Cage ${c?.name ?? id} removed`, details: "" });
  }, [cages, addActivity]);
  const assignCage = useCallback((rabbitId: string, cageId: string | null) => {
    const r = rabbits.find(x => x.id === rabbitId);
    const c = cageId ? cages.find(x => x.id === cageId) : null;
    setRabbits(p => p.map(x => x.id === rabbitId ? { ...x, cageId } : x));
    addActivity({
      date: today(), type: "updated", subject: r?.tagId ?? rabbitId,
      description: c ? `Rabbit ${r?.tagId ?? rabbitId} moved to ${c.name}` : `Rabbit ${r?.tagId ?? rabbitId} unassigned from cage`,
      details: "",
    });
  }, [rabbits, cages, addActivity]);
  const delPig = useCallback((id: string) => {
    const p2 = pigs.find(x => x.id === id);
    setPigs(p => p.filter(x => x.id !== id));
    addActivity({ date: today(), type: "deleted", subject: p2?.tagId ?? id, description: `Pig ${p2?.name || p2?.tagId || id} removed`, details: "" });
  }, [pigs, addActivity]);
  const delEggLog = useCallback((id: string) => {
    const e = eggLogs.find(x => x.id === id);
    setEggLogs(p => p.filter(x => x.id !== id));
    addActivity({ date: today(), type: "deleted", subject: "Egg log", description: `Egg log for ${e?.date ?? id} removed`, details: "" });
  }, [eggLogs, addActivity]);
  const delExpense = useCallback((id: string) => {
    const e = expenses.find(x => x.id === id);
    setExpenses(p => p.filter(x => x.id !== id));
    addActivity({ date: today(), type: "deleted", subject: e?.description ?? "Expense", description: `Expense ${e?.description ?? id} removed`, details: "" });
  }, [expenses, addActivity]);
  const delSale = useCallback((id: string) => {
    const s = sales.find(x => x.id === id);
    setSales(p => p.filter(x => x.id !== id));
    addActivity({ date: today(), type: "deleted", subject: s?.product ?? "Sale", description: `Sale of ${s?.product ?? id} removed`, details: "" });
  }, [sales, addActivity]);
  const delVaccination = useCallback((id: string) => {
    const v = vaccinations.find(x => x.id === id);
    setVaccinations(p => p.filter(x => x.id !== id));
    addActivity({ date: today(), type: "deleted", subject: v?.vaccine ?? "Vaccination", description: `Vaccination record for ${v?.animalGroup ?? id} removed`, details: "" });
  }, [vaccinations, addActivity]);
  ...
  const updateRabbitStatus = useCallback((id: string, status: RabbitRecord["status"], notes: string, lastBredDate: string, expectedKindleDate: string) => {
    const rabbit = rabbits.find(r => r.id === id);
    setRabbits(p => p.map(r => r.id === id ? { ...r, status, lastBredDate, expectedKindleDate } : r));
    addActivity({
      date: today(), type: status === "deceased" ? "death" : "status_change",
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-background" style={{ fontFamily: "var(--font-sans)" }}>
      ...
      <main className="flex-1 overflow-y-auto pb-16">
        ...
        {section === "poultry" && <PoultrySection flocks={flocks} onAdd={addFlock} onEdit={editFlock} onDelete={delFlock} onLogDeath={logBirdDeath} />}
        {section === "rabbits" && <RabbitsSection rabbits={rabbits} cages={cages} onAdd={addRabbit} onEdit={editRabbit} onDelete={delRabbit} onUpdateStatus={updateRabbitStatus} onAddCage={addCage} onDeleteCage={delCage} onAssignCage={assignCage} />}
